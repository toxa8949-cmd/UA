...
export function HeroDestinations({ countries }: { countries: Country[] }) {
  return (
    <div className="mx-auto mt-14 grid max-w-4xl gap-px overflow-hidden rounded-2xl border border-sand-300 bg-sand-300 sm:grid-cols-2 lg:grid-cols-5">
      {countries.map((c) => {
            ...
            <div className="mt-6">
              ...
              {c.average_salary != null && c.currency ? (
                <p className="mt-0.5 text-xs text-slate-500">
                  від {formatMoney(c.average_salary, c.currency)}/міс
                </p>
              ) : (
